  ...
  const newsList = [
    {
      ...
      image: CoverImage,
      content:
        "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras sit amet orci non metus.",
      views: 120,
      likes: 15,
      dislikes: 2,
  ...
  const handleVote = (newsId, type) => {
    ...
    const updatedNews = newsState.map((item) => {
      if (item.id === newsId) {
        ...
        if (currentVote === type) {
          if (type === "like" && newLikes > 0) newLikes--;
          if (type === "dislike" && newDislikes > 0) newDislikes--;
          return { ...item, likes: newLikes, dislikes: newDislikes };
        }

        if (currentVote === "like" && newLikes > 0) newLikes--;
        if (currentVote === "dislike" && newDislikes > 0) newDislikes--;
    ...
    <div className="w-full">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 py-10 grid grid-cols-1 sm:grid-cols-3 gap-6">
        ...
        <div className="sm:col-span-2 space-y-6">
          ...
          <h2 className="text-xl sm:text-3xl font-bold text-[#1E3A8A]">
            {news.title}
          </h2>
            ...
            <div className="flex items-center space-x-1 sm:space-x-2">
              <button onClick={() => handleVote(news.id, "dislike")}>
                <FontAwesomeIcon
                  className="text-lg"
                  icon={
                    userVotes[news.id] === "dislike"
                      ? faThumbsDownSolid
                      : faThumbsDownRegular
                  }
                />
              </button>
              <span className="text-sm">{news.dislikes}</span>
            </div>
          </div>
