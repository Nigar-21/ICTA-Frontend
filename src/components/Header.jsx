  ...
  <nav className='flex gap-2 text-white font-semibold'>
        {navLinks.map((link, index) => (
          <div key={index} className="relative group">
            {link.dropdown ? (
              <>
                <span className="cursor-pointer transition-colors duration-200 hover:bg-[#1E3A8A] py-2 px-4  rounded-full ">{link.name}</span>
                <div className="absolute left-0 top-full mt-2 w-52 bg-white rounded-lg shadow-lg opacity-0 group-hover:opacity-100 invisible group-hover:visible transition-all duration-300 z-10">
              ...
              <Link
                to={link.to}
                className="relative transition-colors duration-200 hover:bg-[#1E3A8A] py-2 px-4 rounded-full"
              >
                {link.name}
              </Link>
            )}
